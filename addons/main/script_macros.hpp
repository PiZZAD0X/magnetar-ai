...
#define DGVAR(varName)    if(isNil "MAI_DEBUG_NAMESPACE") then { MAI_DEBUG_NAMESPACE = []; }; if(!(QUOTE(GVAR(varName)) in MAI_DEBUG_NAMESPACE)) then { PUSH(MAI_DEBUG_NAMESPACE, QUOTE(GVAR(varName))); }; GVAR(varName)
#define DVAR(varName)     if(isNil "MAI_DEBUG_NAMESPACE") then { MAI_DEBUG_NAMESPACE = []; }; if(!(QUOTE(varName) in MAI_DEBUG_NAMESPACE)) then { PUSH(MAI_DEBUG_NAMESPACE, QUOTE(varName)); }; varName
...
#define MAI_DEPRECATED(arg1,arg2,arg3) WARNING_3("%1 is deprecated. Support will be dropped in version %2. Replaced by: %3",arg1,arg2,arg3)
